...
impl<T: Float + AddAssign + num_traits::FloatConst + num_traits::FromPrimitive + std::ops::SubAssign> Stretcher<T> {
    ...
    pub fn process(&mut self, input: &[T], output: &mut [T]) {
        ...
        // Create references to state vectors
        let phase_acc = &mut self.phase_accumulator;
        let last_ph = &mut self.last_phase;
        let last_mag = &mut self.last_magnitude;
        ...
        let frequency_smoothing = self.config.frequency_smoothing;
        let fft_size = self.fft_size;
        let hop_size = self.hop_size;

        let process_fn = move |spectrum: &mut [Complex<T>]| {
            let spectrum_size = spectrum.len();
            let bin_to_freq = T::one() / T::from(fft_size).unwrap();
            ...
            for i in 0..spectrum_size {
                ...
                if last_ph[i] == T::zero() {
                    // First frame: initialize phase accumulator
                    phase_acc[i] = phase;
                } else {
                    ...
                    let mut wrapped_phase_diff = phase_diff - two_pi *
                        ((phase_diff + pi) / two_pi).floor();

                    // Ensure we're within [-π, π]
                    if wrapped_phase_diff < -pi {
                        wrapped_phase_diff += two_pi;
                    } else if wrapped_phase_diff > pi {
                        wrapped_phase_diff -= two_pi;
                    }

                    // Calculate true frequency
                    let bin_freq = T::from(i).unwrap() * bin_to_freq;
                    let true_freq = bin_freq + wrapped_phase_diff / (two_pi * T::from(hop_size).unwrap());

                    // Apply pitch shift
                    let shifted_freq = true_freq * pitch_shift_factor;

                    // Calculate phase increment
                    let phase_increment = shifted_freq * two_pi * T::from(hop_size).unwrap();

                    // Update phase accumulator
                    phase_acc[i] += phase_increment;
                }
                ...
                let output_phase = if phase_locking > T::zero() {
                    ...
                    let free_phase = phase * pitch_shift_factor;
                ...
                let output_magnitude = if transient_preservation > T::zero() {
                    ...
                    let magnitude_ratio = if last_mag[i] > T::from(1e-10).unwrap() {
                ...
                let final_magnitude = if frequency_smoothing > T::zero() && i > 0 && i < spectrum_size - 1 {
                    let smooth_factor = <T as NumCast>::from(frequency_smoothing).unwrap();
                    ...
                    let avg_mag = (prev_mag + output_magnitude + next_mag) / T::from(3.0).unwrap();
...
impl<T: Float + AddAssign + num_traits::FloatConst + num_traits::FromPrimitive + SubAssign> RealtimeStretcher<T> {
    ...
    pub fn process_buffer(&mut self, input: &[T], output: &mut [T]) {
        ...
        let output_hop = (T::from(hop_size).unwrap() / stretch_factor);
        let output_hop = output_hop.round().to_usize().unwrap();

        let mut output_pos = 0;

        while self.input_buffer.len() >= fft_size {
            // Process one frame
            let mut frame_output = vec![T::zero(); fft_size];
            self.stretcher
                .process(&self.input_buffer[0..fft_size], &mut frame_output);

            // Overlap-add to output buffer
            let end = std::cmp::min(output_pos + fft_size, output_len);
            for i in 0..(end - output_pos) {
                output[output_pos + i] = output[output_pos + i] + frame_output[i];
            }

            // Advance input position
            let drain_size = hop_size.min(self.input_buffer.len());
            self.input_buffer.drain(0..drain_size);
...
mod tests {
    ...
    fn test_stretcher_identity() {
        ...
        // Check that the output approximates the input (allowing for latency)
        let latency = stretcher.latency();
        let start = latency + 512;
        let end = output.len().min(input.len() - 512);

        for i in start..end {
            if output[i].abs() > 0.001 { // Only check non-silent regions
                assert!(
                    (input[i - latency] - output[i]).abs() < 0.2,
    ...
    fn test_stretcher_pitch_shift() {
        ...
        stretcher.set_config(StretchConfig {
            ...
            transient_preservation: 0.0,
            frequency_smoothing: 0.0,
            phase_locking: 1.0, // Full phase locking for better coherence
        });

        // Create a test signal with stable frequency (440Hz at 44.1kHz sample rate)
        ...
        for i in 0..num_samples {
            input[i] = (i as f32 * frequency * 2.0 * PI / sample_rate as f32).sin();
        }
        ...
        // Check that the output has reasonable energy
        let latency = stretcher.latency();
        let start = latency + 1024;
        let end = output.len().min(input.len() - 1024);
        let output_energy: f32 = output[start..end]
            .iter()
            .map(|&x| x * x)
            .sum::<f32>() / (end - start) as f32;

        // Input energy should be approximately 0.5 (since sin^2 averages to 0.5)
        // Allow for significant loss due to windowing and processing
        assert!(output_energy > 0.1, "Output energy ({}) too low", output_energy);
    }

    #[test]
    fn test_realtime_stretcher() {
        let mut stretcher = RealtimeStretcher::<f32>::new(1024, 4, 4096);
        ...
        // Process
        stretcher.process_buffer(&input, &mut output);

        // Check that the output approximates the input (allowing for latency)
        let latency = stretcher.latency();
        let start = latency + 256;
        let end = output.len().min(input.len() - 256);
